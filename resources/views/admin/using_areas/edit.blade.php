...
@section('title', 'Kullanım Alanı Güncelleme')
...
<!--begin::App Content Header-->
        <div class="app-content-header">
          ...
          <div class="container-fluid">
            <!--begin::Row-->
            <div class="row">
              <div class="col-sm-6"><h3 class="mb-0">Kullanım Alanı Güncelleme</h3></div>
              <div class="col-sm-6">
                <ol class="breadcrumb float-sm-end">
                  <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">Anasayfa</a></li>
                  <li class="breadcrumb-item active" aria-current="page">Kullanım Alanı Yönetimi</li>
                </ol>
              </div>
            </div>
            <!--end::Row-->
        ...
        <div class="app-content">
                ...
                <div class="card">
                    ...
                    <?php 
                        foreach($using_areas as $area){
                            $area_id[$area->lang] = $area->using_areas_id;
                            $seo_url[$area->lang] = $area->seo_url;
                            $title_1[$area->lang] = $area->title_1;
                            $title_2[$area->lang] = $area->title_2;
                            $short_description[$area->lang] = $area->short_description;
                            $description[$area->lang] = $area->description;
                            $image[$area->lang] = $area->image;
                            $alt[$area->lang] = $area->alt;
                            $used_products[$area->lang] = explode(',', $area->used_products) ?? [];
                            $seo_title[$area->lang] = $area->seo_title;
                            $seo_description[$area->lang] = $area->seo_description;
                            $seo_keywords[$area->lang] = $area->seo_keywords;
                            $sort[$area->lang] = $area->sort;
                        }
                    ?>
                    <div class="card-body">
                        <form action="{{ route('admin.using_areas.store') }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            <div class="tab-content" id="myTabContent">
                                ...
                                <div class="tab-pane fade {{ $loop->first ? 'show active' : '' }}" id="tab-{{ $language->id }}" role="tabpanel" aria-labelledby="tab-{{ $language->id }}-tab">
                                    <div class="card-body" style="display:grid; grid-template-columns: 1fr 1fr 1fr; gap: 20px;">
                                        ...
                                        <input type="hidden" name="using_areas_id" value="{{ $area_id[$language->lang_code] }}">
                                        <div class="mb-3">
                                        ...
                                        <div class="mb-3">
                                            ...
                                            @if(isset($image[$language->lang_code]))
                                                <img src="{{ $languages[0]->domain .'/'. getFolder(['uploads_folder', 'using_area_images_folder'], $language->lang_code) . '/' . $image[$language->lang_code] }}" alt="{{ $alt[$language->lang_code] ?? '' }}" class="img-thumbnail mt-2" width="100">
